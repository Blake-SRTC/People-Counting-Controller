...
from mylib.api import sendPostTotal
...
def run():
    ...
    while True:
        ...
        if np.size(empty) > e1 or np.size(empty1) > e2:
            ...
            # Enviar el total como informacion adicional para el dashboard ///////////////////////////////
            respuesta_server_total = sendPostTotal(total)
            print("Servidor total: {} ".format(respuesta_server_total))

        # nuevo envio de datos a servidor para registro a base de datos //////////////////////////////////
        if entrada == 1:
            respuesta_server = sendPost(entrada, 0)
            print("Servidor responde entrada: {} ".format(respuesta_server))
            entrada = 0
        if salida == 1:
            respuesta_server = sendPost(0, salida)
            print("Servidor responde salida: {} ".format(respuesta_server))
            salida = 0
